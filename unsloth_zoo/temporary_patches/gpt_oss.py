...
def swiglu_torch_backward(pre_act, alpha, limit, g1):
    ...
    if limit is not None:
        mask_g = g <= limit
        mask_l = l.abs() <= limit
        g_bar = torch.where(mask_g, g, limit)
        l_bar = torch.where(mask_l, l, l.sign() * limit)
    else:                                            # no clipping
        mask_g = mask_l = torch.ones_like(g, dtype=bool)
        g_bar, l_bar = g, l

    σ   = torch.sigmoid(alpha * g_bar)
    dg  = (σ + alpha * g_bar * σ * (1 - σ)) * (l_bar + 1)
    dl  = g_bar * σ
...
def torch_native_forward_float32(
    self,
    hidden_states: torch.Tensor,
    router_indices = None,
